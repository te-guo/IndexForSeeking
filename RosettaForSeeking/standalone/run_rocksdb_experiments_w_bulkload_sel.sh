...
if [ "$#" != "5" ]; then
    echo "Usage: $0 [path to experiment binary] [path to workload generator binary] [scratch directory to perform experiments in] [scratch directory to store experiment data in] [scratch dir for bulk data tmp]"
    exit
fi
...
BULK_TMP_DIR="$(readlink -f $1)" && shift
MAX_JOBS=1

EMAIL_ME="wilsonqin@g.harvard.edu"
...
experiment() {
	...
	surftype=$1 && shift
        ...
        "$EXPER_BIN" "surf" "write" "$nkeys" "$nqrys" ./db/ "$BULK_TMP_DIR" ./my_data/ "$membudg" 0 "$surftype" >> ./experiment_result && "$EXPER_BIN" "surf" "read" "$nkeys" "$nqrys" ./db/ "$BULK_TMP_DIR" ./my_data/ "$membudg" 1 "$surftype" >> ./experiment_result
    elif [ $filter = "DST" ]; then
        echo -e "\tBfs diffidence:\t$bfsdiff" >> ./experiment_result
        echo -e "### END EXPERIMENT DESCRIPTION ###\n\n" >> ./experiment_result
        "$EXPER_BIN" "dst" "write" "$nkeys" "$nqrys" ./db/ "$BULK_TMP_DIR" ./my_data/ "$membudg" 0 "$bfsdiff" >> ./experiment_result && "$EXPER_BIN" "dst" "read" "$nkeys" "$nqrys" ./db/ "$BULK_TMP_DIR" ./my_data/ "$membudg" 1 "$bfsdiff" >> ./experiment_result
    elif [ $filter = "None" ]; then
        echo -e "### END EXPERIMENT DESCRIPTION ###\n\n" >> ./experiment_result
        "$EXPER_BIN" "none" "write" "$nkeys" "$nqrys" ./db/ "$BULK_TMP_DIR" ./my_data/ "$membudg" 0 >> ./experiment_result && "$EXPER_BIN" "none" "read" "$nkeys" "$nqrys" ./db/ "$BULK_TMP_DIR" ./my_data/ "$membudg" 1 >> ./experiment_result
    elif [ $filter = "Prefix" ]; then
	prefix=$1 && shift
        echo -e "### END EXPERIMENT DESCRIPTION ###\n\n" >> ./experiment_result
        "$EXPER_BIN" "prefix" "write" "$nkeys" "$nqrys" ./db/ "$BULK_TMP_DIR" ./my_data/ "$membudg" 0 "$prefix" >> ./experiment_result && "$EXPER_BIN" "prefix" "read" "$nkeys" "$nqrys" ./db/ "$BULK_TMP_DIR" ./my_data/ "$membudg" 1 "$prefix" >> ./experiment_result
    fi
    ...
    if [ ! -z "$EMAIL_ME" ]; then
        echo "RUN ENDED: membudg: $membudg, mrange: $mrange, filter: $filter." | mail -s "run on $HOSTNAME" $EMAIL_ME
    fi
}
...
qdist_arr=("uniform")
mrange_arr=("0.001")
#mrange_arr=("0.0001" "0.0003" "0.001" "0.003" "0.01" "0.03" "0.1" "0.3")
#mrange_arr=("0.00000002" "0.00000004" "0.00000008" "0.00000016" "0.00000032" "0.000001" "0.00001" "0.0001" "0.001" "0.01" "0.1")
# 1
mrange_arr=(2 4 8 16 32 64 128 256 512 1000 2000 4000 10000)
#mrange_arr=(10000 100000 1000000 10000000 30000000 50000000 70000000 90000000)
#
dfsdiff_arr=(10000)
bfsdiff_arr=(64)
pqratio_arr=(0.0)
