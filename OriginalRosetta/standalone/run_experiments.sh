...
experiment() {
    ...
    if [ $filter = "SuRF" ]; then
        ...
        "$REPO_DIR"/standalone/run_workload ./my_data/data.txt ./my_data/txn.txt ./my_data/upper_bound.txt "$membudg" "$filter" "$surfhlen" "$surfrlen" >> ./experiment_result
    else
        dfsdiff=$1 && shift
        bfsdiff=$1 && shift
        cutoff=$1 && shift
        ...
        echo -e "### END EXPERIMENT DESCRIPTION ###\n\n" >> ./experiment_result
        "$REPO_DIR"/standalone/run_workload ./my_data/data.txt ./my_data/txn.txt ./my_data/upper_bound.txt "$membudg" "$filter" "$dfsdiff" "$bfsdiff" "$cutoff" >> ./experiment_result
    fi
...
keylen_arr=(64)
nqrys_arr=(100000)
kdist_arr=("uniform" "zipfian") # "normal" "zipfian")
qdist_arr=("uniform" "correlated") # "normal" "zipfian" "adversary")
mrange_arr=(2048)
dfsdiff_arr=(10000)
bfsdiff_arr=(64)
pqratio_arr=(0.0)
membudg_arr=(10 14 18)
surfhlen_arr=(0 4)
surfrlen_arr=(0 4)
expdir_arr=()
corrd_arr=(1)
cutoff_arr=(0 4 8 16 32)
