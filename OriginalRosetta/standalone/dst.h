...
class SurfFilter final: public Filter {
    private:
        surf::SuRF* s_;
        size_t hash_suffix_len_,
               real_suffix_len_;
        surf::SuffixType suf_type_;

    public:
        SurfFilter(size_t hash_suffix_len, size_t real_suffix_len, surf::SuffixType suf_type);
        ~SurfFilter();
        ...
        bool Query(const Bitwise &from, const Bitwise &to);
        pair<uint8_t*, size_t> serialize() const;
        static pair<SurfFilter*, size_t> deserialize(uint8_t* ser);
};
#endif
