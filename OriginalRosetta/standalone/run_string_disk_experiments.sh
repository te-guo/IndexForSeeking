...
if [ "$#" != "2" ]; then
    echo "Usage: $0 [scratch directory to perform experiments in] [path to email list]"
    exit
fi
...
experiment() {
    ...
    echo python "$REPO_DIR"/workload_gen/gen_strings.py "$EMAILS" "./my_data" "$nkeys" "$nqrys" 
    python "$REPO_DIR"/workload_gen/gen_strings.py "$EMAILS" "./my_data" "$nkeys" "$nqrys" 
    if [ $filter = "SuRF" ]; then
        surfhlen=$1 && shift
        surfrlen=$1 && shift
        ...
        echo "$REPO_DIR"/standalone/run_workload_string_disk ./my_data/data.txt ./my_data/txn.txt ./my_data/upper_bound.txt "$membudg" "$filter" "$surfhlen" "$surfrlen"
        "$REPO_DIR"/standalone/run_workload_string_disk ./my_data/data.txt ./my_data/txn.txt ./my_data/upper_bound.txt "$membudg" "$filter" "$surfhlen" "$surfrlen" >> ./experiment_result
    else
        dfsdiff=$1 && shift
        bfsdiff=$1 && shift
        cutoff=$1 && shift
        ...
        echo -e "### END EXPERIMENT DESCRIPTION ###\n\n" >> ./experiment_result
        echo "$REPO_DIR"/standalone/run_workload_string_disk ./my_data/data.txt ./my_data/txn.txt ./my_data/upper_bound.txt "$membudg" "$filter" "$dfsdiff" "$bfsdiff" "$cutoff" 
        "$REPO_DIR"/standalone/run_workload_string_disk ./my_data/data.txt ./my_data/txn.txt ./my_data/upper_bound.txt "$membudg" "$filter" "$dfsdiff" "$bfsdiff" "$cutoff" >> ./experiment_result
    fi
    ...
    for nqrys in "${nqrys_arr[@]}"; do
        for dfsdiff in "${dfsdiff_arr[@]}"; do
			...
			for fracfull in "${fracfull_arr[@]}"; do
			    expdir=$(mktemp -d "$WORKDIR/dst_experiment.XXXXXXX")
			    expdir_arr+=("$expdir")
