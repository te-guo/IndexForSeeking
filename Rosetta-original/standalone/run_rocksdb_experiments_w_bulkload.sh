...
experiment() {
	...
	surftype=$1 && shift
        ...
        echo -e "### END EXPERIMENT DESCRIPTION ###\n\n" >> ./experiment_result
        "$EXPER_BIN" "none" "write" "$nkeys" "$nqrys" ./db/ "$BULK_TMP_DIR" ./my_data/ "$membudg" 0 "$bfsdiff" >> ./experiment_result && "$EXPER_BIN" "none" "read" "$nkeys" "$nqrys" ./db/ "$BULK_TMP_DIR" ./my_data/ "$membudg" 1 "$bfsdiff" >> ./experiment_result
